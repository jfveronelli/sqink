...
class SqliteNoteProvider(LocalNoteProvider):
    ...
    def get(self, uuid):
        query= "SELECT uuid, lastModified, createdOn, title, tags, starred, text, html FROM Note WHERE active = 1 AND uuid = ?;"
        cursor= self.__connection.cursor()
    ...
    def remove(self, note):
        ...
        with self.__connection:
            ...
            self.__connection.execute("INSERT INTO Note VALUES (?, ?, 0, NULL, NULL, NULL, NULL, NULL, NULL);", (uuid, note.lastModified))

        photoPath= self.__photoPath(uuid)
        if isfile(photoPath):
            remove(photoPath)

    def __flatten(self, tags):
        if not tags:
            return None
        txt= tags[0]
    ...
    def __inflate(self, tags):
        return tags.split("\n") if tags else []

    def __likenize(self, text):
        return "%" + text + "%"
