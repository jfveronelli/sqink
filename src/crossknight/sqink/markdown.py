...
def renderHtml(note):
    ...
    if note.createdOn or note.lastModified:
        ...
        if note.createdOn:
            dates+= " <img src=\"images/created-16x16.png\"/> <span>" + _formatDatetime(note.createdOn) + "</span> &nbsp"
    ...
    if diagram:
        javascript= "<script type=\"text/javascript\" src=\"js/mermaid.min.js\"></script>" +\
                ...
                "mermaid.initialize({startOnLoad:true,flowchart:{useMaxWidth:false},sequenceDiagram:{useMaxWidth:false}});" +\
                "</script>"
